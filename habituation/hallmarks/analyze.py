...
def audit(root,spec):
    ...
    for name,digest in m['source_sha256'].items():
        source=root/'runner_snapshot.py' if name.endswith('/hallmarks/run.py') else (root/'entrypoint_snapshot.py' if any(name.endswith('/hallmarks/'+n) for n in ['run_partial.py','run_frequency.py']) else base.ROOT/name)
    ...
    base.require(set(pop)==({'A'}|set(spec['B_populations']) if m['part']=='novel' else {'A'}),'Population coverage')
    canonical=json.loads((HERE.parent/'protocols/main_v1.json').read_text())
    base.require(cells['readouts']==canonical['readouts'],'Readout identity drift')
    ...
    for seed in spec['seeds']:
        ...
        for row in g.itertuples():
            ...
            if row.kind=='response' and row.phase=='train' and (row.case in ['fast12','novel_train'] or (row.case=='h3_partial' and row.block==1) or (row.case=='frequency24' and row.pulse==0)):
                actual_signatures[(seed,row.pulse)]=base.sha(path)
    base.require(len(r)=={'main':261,'novel':72,'partial':108,'frequency':84}[m['part']],'Response coverage')
...
def main_scores(d,spec):
    ...
    for seed in spec['seeds']:
        ...
        deltaD=tm['fast12']['D']-tm['strong12']['D'] if all(tm[k]['D'] is not None for k in ['fast12','strong12']) else None
        usable=tm['fast12']['initial_usable'] and tm['strong12']['initial_usable'] and deltaD is not None
...
def partial_scores(d,spec):
    scores=[];trains=[]
    for seed in spec['seeds']:
        g=d[d.seed==seed];values=[];blocks=[]
        for block in [1,2,3]:
            v=g[(g.phase=='train')&(g.block==block)].sort_values('pulse').aBN1.to_numpy();values.append(v)
            m=train_metrics(v);blocks.append(m);trains.append({'seed':seed,'case':f'partial_cycle{block}',**m})
        target=.5*blocks[0]['initial']
        half=[next((i+1 for i in range(len(v)-2) if np.all(v[i:i+3]<=target)),None) for v in values]
        eligible=blocks[0]['initial_usable'] and blocks[0]['decrement'] and all(blocks[i]['initial']>blocks[i-1]['late'] for i in [1,2])
        extent=blocks[2]['early']<=blocks[1]['early']<=blocks[0]['early'] and blocks[0]['early']-blocks[2]['early']>=1-1e-12
        speed=all(x is not None for x in half) and half[2]<=half[1]<=half[0] and half[2]<=half[0]-1
        scores.append({'seed':seed,'hallmark':'H3_partial','eligible':bool(eligible),'initial_by_cycle':[x['initial'] for x in blocks],
                       'early_by_cycle':[x['early'] for x in blocks],'D_by_cycle':[x['D'] for x in blocks],
                       'half_trial_own_baseline':[x['half_trial'] for x in blocks],'half_trial_common_baseline':half,
                       'early_count_enhancement':bool(extent),'common_baseline_speed_enhancement':bool(speed),
                       'supported':bool(eligible and (extent or speed))})
    return scores,pd.DataFrame(trains)


def frequency_scores(d,fast_train,fast_profiles,spec):
    scores=[];trains=[];profiles=[]
    for seed in spec['seeds']:
        v=d[(d.seed==seed)&(d.phase=='train')].sort_values('pulse').aBN1
        slow=train_metrics(v);fast=fast_train[(fast_train.seed==seed)&(fast_train.case=='fast24')].iloc[0].to_dict()
        trains.append({'seed':seed,'case':'frequency24',**slow});rec={}
        for name,frame,tm in [('fast24',fast_profiles,fast),('frequency24',d,slow)]:
            h=frame[(frame.seed==seed)&(frame.case==name)&(frame.rest_ms.isin(spec['recovery_ms']))].sort_values('rest_ms')
            base.require(list(h.rest_ms)==spec['recovery_ms'],'Supplement probe coverage')
            rec[name]=recovery_metrics(tm,h.rest_ms.to_numpy(),h.aBN1.to_numpy())
            for i,row in enumerate(h.itertuples()):profiles.append({'seed':seed,'case':name,'rest_ms':row.rest_ms,'aBN1':row.aBN1,
                'F':rec[name]['F'][i],'relative_initial':rec[name]['relative_initial'][i],'initial':tm['initial'],'late':tm['late']})
        eligible=all(t['initial_usable'] and t['decrement'] and t['plateau'] and t['initial']>t['late'] for t in [fast,slow])
        indices=[spec['recovery_ms'].index(t) for t in [500.,1000.,2000.]]
        delta=float(np.median([rec['fast24']['F'][i]-rec['frequency24']['F'][i] for i in indices])) if eligible else None
        hi=rec['fast24'];lo=rec['frequency24'];earlier=strictly_earlier(hi['half_lost'],lo['half_lost'])
        reverse=strictly_earlier(lo['half_lost'],hi['half_lost'])
        abs_hi=strictly_earlier(hi['eighty_initial'],lo['eighty_initial']);abs_lo=strictly_earlier(lo['eighty_initial'],hi['eighty_initial'])
        support=bool(eligible and earlier and delta>=.1);opposite=bool(eligible and reverse and delta<=-.1)
        scores.append({'seed':seed,'hallmark':'H4b_qualified','eligible':bool(eligible),'median_fraction_advantage':delta,
                       'fast_half_interval':hi['half_lost'],'slow_half_interval':lo['half_lost'],
                       'fast_80_interval':hi['eighty_initial'],'slow_80_interval':lo['eighty_initial'],
                       'supported':support,'opposite_screen':opposite,'absolute_80_high_faster':abs_hi,'absolute_80_low_faster':abs_lo,
                       'normalization_conflict':bool((support and abs_lo) or (opposite and abs_hi))})
    return scores,pd.DataFrame(trains),pd.DataFrame(profiles)


def main():
    ap=argparse.ArgumentParser();ap.add_argument('--main',type=Path);ap.add_argument('--novel',type=Path);ap.add_argument('--partial',type=Path);ap.add_argument('--frequency',type=Path);ap.add_argument('--output',type=Path,required=True);args=ap.parse_args()
    base.require(any(x is not None for x in [args.main,args.novel,args.partial,args.frequency]),'No input runs')
    base.require(args.frequency is None or args.main is not None,'Frequency supplement requires matched main controls')
    ...
    partial_train=None;frequency_train=None;frequency_profiles=None
    for part,path in [('novel',args.novel),('main',args.main),('partial',args.partial),('frequency',args.frequency)]:
        if path is None:continue
        part_spec=json.loads((HERE/(part+'_protocol.json')).read_text()) if part in ['partial','frequency'] else spec
        path=path.resolve();d,evidence,check,sig=audit(path,part_spec)
        ...
        if part=='main':
            ...
            s,partial_train=partial_scores(d,part_spec);scores+=s
            freeze=json.loads((HERE/'partial_freeze.json').read_text());m=json.loads((path/'manifest.json').read_text())
            base.require(datetime.fromisoformat(freeze['created_utc'])<datetime.fromisoformat(m['supplement_started_utc']),'Supplement not frozen before running')
        elif part=='frequency':
            s,frequency_train,frequency_profiles=frequency_scores(d,train,recovery,part_spec);scores+=s
            freeze=json.loads((HERE/'frequency_freeze.json').read_text());m=json.loads((path/'manifest.json').read_text())
            base.require(datetime.fromisoformat(freeze['created_utc'])<datetime.fromisoformat(m['supplement_started_utc']),'Frequency supplement not prospectively frozen')
        else:scores+=novel_scores(d,spec)
    partial_probe_equivalence=None
    if all(k in frames for k in ['main','partial']):
        partial_probe_equivalence={}
        for seed in spec['seeds']:
            a=frames['main'];b=frames['partial']
            f1=a[(a.seed==seed)&(a.case=='fast12')&(a.phase=='recovery')&(a.rest_ms==2000)].event_file.item()
            f2=b[(b.seed==seed)&(b.block==2)&(b.pulse==0)].event_file.item()
            same=pd.read_parquet(args.main/f1).equals(pd.read_parquet(args.partial/f2))
            partial_probe_equivalence[str(seed)]=same
        base.require(all(partial_probe_equivalence.values()),'Continuous 2s recovery differs from independent branch: review restore state')
    allchecks=pd.concat(checks,ignore_index=True);input_valid=bool(allchecks.matches_expected.all())
    paired_equivalence=signatures['main']==signatures['novel'] if all(k in signatures for k in ['main','novel']) else None
    partial_equivalence=signatures['main']==signatures['partial'] if all(k in signatures for k in ['main','partial']) else None
    frequency_initial_equivalence=all(signatures['main'][k]==v for k,v in signatures['frequency'].items()) if 'frequency' in signatures else None
    groups={}
    for s in scores:
    ...
    report={'status':'completed_audit','created_utc':datetime.now(timezone.utc).isoformat(),'analyzer_sha256':base.sha(Path(__file__)),
            ...
            'main_vs_partial_first_cycle_event_hashes_equal':partial_equivalence,
            'continuous_partial_matches_independent_2s_probe':partial_probe_equivalence,
            'frequency_initial_event_hashes_equal':frequency_initial_equivalence,
    ...
    if partial_train is not None:partial_train.to_csv(out/'partial_training_metrics.csv',index=False)
    if frequency_train is not None:frequency_train.to_csv(out/'frequency_training_metrics.csv',index=False);frequency_profiles.to_csv(out/'frequency_recovery_profiles.csv',index=False)
