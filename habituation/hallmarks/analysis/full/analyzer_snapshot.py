...
def audit(root,spec):
    ...
    for name,digest in m['source_sha256'].items():
        source=root/'runner_snapshot.py' if name.endswith('/hallmarks/run.py') else (root/'entrypoint_snapshot.py' if name.endswith('/hallmarks/run_partial.py') else base.ROOT/name)
    ...
    for seed in spec['seeds']:
        ...
        for row in g.itertuples():
            ...
            if row.kind=='response' and row.phase=='train' and (row.case in ['fast12','novel_train'] or (row.case=='h3_partial' and row.block==1)):
                actual_signatures[(seed,row.pulse)]=base.sha(path)
    base.require(len(r)=={'main':261,'novel':72,'partial':108}[m['part']],'Response coverage')
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--main',type=Path);ap.add_argument('--novel',type=Path);ap.add_argument('--partial',type=Path);ap.add_argument('--output',type=Path,required=True);args=ap.parse_args()
    base.require(any(x is not None for x in [args.main,args.novel,args.partial]),'No input runs')
    ...
    partial_train=None
    for part,path in [('novel',args.novel),('main',args.main),('partial',args.partial)]:
        if path is None:continue
        part_spec=json.loads((HERE/'partial_protocol.json').read_text()) if part=='partial' else spec
