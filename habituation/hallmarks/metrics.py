...
def expected_schedule(part,spec):
    ...
    if part=='main':
        ...
        train('frequency24',spec['slow_interval_ms'],end=spec['n_train']);recover(clock,'frequency24')
    elif part=='partial':
        for block in [1,2,3]:
            if block>1:add('quiet',spec['between_cycles_ms'],'h3_partial','cycle_rest',block)
            train('h3_partial',500,block)
    else:
